...
export default function Project() {
  ...
  // if (projectyQuery.status === "loading") return <h1>Loading...</h1>
  // if (projectyQuery.status === "error") return <h1>Error</h1>
  ...
  //       <Script src="https://kit.fontawesome.com/b24cab7e32.js" crossorigin="anonymous"></Script>
  //       <Head>
  //         <title>{projectyQuery.data?.data?.name}</title>
  //         <meta
  //           name="description"
  //           content={`An overview of ${projectyQuery.data?.data?.name} crypto project.`}
  //         />
  //         <meta
  //           name="keywords"
  //           content="crypto dashboard, dashboard crypto, crypto data analytics, cryptocurrency research, blockchain, bitcoin, ethereum"
  ...
  //       <div className='max-w-6xl mx-auto flex flex-col items-center justify-center sm:flex-row sm:space-x-8 p-4 overflow-y-auto mb-20'>
  //             <Image
  //               src={projectyQuery.data?.data?.image}
  //               alt={projectyQuery.data?.data?.name}
  //               width={300}
  //               height={300}
  //               className='object-cover my-4 md:my-2 w-[150px] md:w-[300px]'
  ...
  //                 <Rating 
  //                   className='text-yellow-500 ml-2'
  //                   initialRating={parseInt(projectyQuery.data?.data?.rating)}
  //                   readonly
  //                   fullSymbol="fa fa-star"
  //                   emptySymbol="fa fa-star-o"
  //                   fractions={2}
  ...
  //                 <div className='flex space-x-2 items-center mt-2'>
  //                   <FontAwesomeIcon icon={faArrowUpRightFromSquare} className="h-4 w-4"/>
  //                   <a href={projectyQuery.data?.data?.website} target="_blank" className='text-blue-500 underline hover:text-blue-700'>Visit {projectyQuery.data.data.name}</a>
  //                 </div>
  //                   <Link href="/" className='mt-2 text-blue-500 underline hover:text-blue-700'>Back</Link>
  //             </div>  
  //       </div>
